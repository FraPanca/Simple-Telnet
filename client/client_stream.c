...
int main(int argc, char** argv) {
	...
	int sd, port, nread, countArgs, countCmd;	
	char cmd[MAX_DIM_CMD*(MAX_NUM_ARGS+2)], buf[MAX_DIM_BUF];
   ...
   if(connect(sd, (struct sockaddr *) &serveraddr, sizeof(serveraddr)) < 0) {
   	perror("[CLIENT] : Errore -> Non è stato possibile effettuare la connect");
   	exit(4);
   }
